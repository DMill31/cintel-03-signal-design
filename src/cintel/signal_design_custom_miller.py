"""
signal_design_custom_miller.py - Project script .
...
Paths (relative to repo root)
    ...
    INPUT FILE: data/system_metrics_miller.csv
    OUTPUT FILE: artifacts/signals_custom_miller.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.signal_design_custom_miller
...
DATA_FILE: Final[Path] = DATA_DIR / "system_metrics_miller.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "signals_custom_miller.csv"
...
def main() -> None:
    ...
    # ----------------------------------------------------
    # We will define both a request_volume signal and a throughput signal.
    # In this example, request_volume is just the requests column
    # throughput will be the requests / total_latency_ms
    request_volume_signal_recipe: pl.Expr = pl.col("requests").alias("request_volume")

    throughput_rps_signal_recipe: pl.Expr = (
        (pl.col("requests") / pl.col("total_latency_ms")) * 1000
    ).alias("throughput_rps")  # requests per second
    ...
    # Create an hour time feature by extracting the hour from the timestamp column
    # This shows that we can also create time-based features from a timestamp column
    hour_signal_recipe: pl.Expr = (
    ...
    df_with_signals: pl.DataFrame = df.with_columns(
        [
            ...
            request_volume_signal_recipe,
            throughput_rps_signal_recipe,
    ...
    LOG.info(
        "Created signal columns: error_rate, avg_latency_ms, request_volume, throughput_rps, poor_experience, hour_of_day"
    )
